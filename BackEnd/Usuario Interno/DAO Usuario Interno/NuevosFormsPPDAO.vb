...
Public Class NuevosFormsPPDAO


    Private Shared _instancia As NuevosFormsPPDAO = Nothing
    ''' <summary>
    ''' Propiedad de solo lectura para obtener una instancia única de la clase NuevosFormsPPDAO.
    ''' </summary>
    Public Shared ReadOnly Property Instancia As NuevosFormsPPDAO
        Get
            If _instancia Is Nothing Then
                _instancia = New NuevosFormsPPDAO()
            End If
            Return _instancia
        End Get
    End Property
    ...
    '' Busca las inscripción de productores primarios en la base de datos y los devuelve en una colección.
    ...
    Public Function AEOcargarInscripPP(ByVal region As String) As Collection
        ...
        Try
            ...
            cn.Open()
            Dim sql = "Select CODIGO, NOMBRE, CEDULA, FECHA_ENVIO from GC_PRODUCTOR WHERE REGION = @VALOR1 AND AREA_REVISION = 1"
    ...
    Public Function DirecRegCargarInscripPP(ByVal region As String) As Collection
        ...
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "Select CODIGO, NOMBRE, CEDULA, FECHA_ENVIO from GC_PRODUCTOR WHERE REGION = @VALOR1 AND AREA_REVISION = 2"
    ...
    Public Function DirecMercCargarInscripPP() As Collection
        ...
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "Select CODIGO, NOMBRE, CEDULA, FECHA_ENVIO from GC_PRODUCTOR WHERE AREA_REVISION = 3"
            Dim cmd As New SqlCommand(sql, cn)
            Dim dr As SqlDataReader
            dr = cmd.ExecuteReader()
            While dr.Read
    ...
    Public Function ComerCargarInscripPP() As Collection
        ...
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "Select CODIGO, NOMBRE, CEDULA, FECHA_ENVIO from GC_PRODUCTOR WHERE AREA_REVISION = 4"
            Dim cmd As New SqlCommand(sql, cn)
            Dim dr As SqlDataReader
            dr = cmd.ExecuteReader()
            While dr.Read
    ...
    ''' <returns>Una colección de objetos Inscripciones de los productores primarios.</returns>
    Public Function buscarInscripcionPP(ByVal terminoBusqueda As String) As Collection
        Dim lista As New Collection
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            ...
            Dim sql = "SELECT CODIGO, NOMBRE, CEDULA FROM GC_PRODUCTOR WHERE CODIGO LIKE @TERMINO OR NOMBRE LIKE @TERMINO OR CEDULA LIKE @TERMINO"
    ...
    ''' <returns>Una colección de objetos Inscripciones de los productores primarios.</returns>
    Public Function buscarArchivoPP(ByVal terminoBusqueda As String) As Collection
        Dim lista As New Collection
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "SELECT * FROM VW_GC_PP_HISTORIAL WHERE CODIGO LIKE @TERMINO OR NOMBRE LIKE @TERMINO OR CEDULA LIKE @TERMINO"
    ...
    Public Function RegArchivoPP(ByVal region As String) As Collection
        ...
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "SELECT * FROM VW_GC_PP_HISTORIAL WHERE REGION = @VALOR1 AND COD_ESTADO IN (4, 5)"
    ...
    Public Function ArchivosPP() As Collection
        ...
        Try
            Dim cn As New SqlConnection(ConexionBD.cadenaConexionInscripciones)
            cn.Open()
            Dim sql = "SELECT * FROM VW_GC_PP_HISTORIAL WHERE COD_ESTADO IN (4, 5)"
            Dim cmd As New SqlCommand(sql, cn)
            Dim dr As SqlDataReader
            dr = cmd.ExecuteReader()
            While dr.Read
